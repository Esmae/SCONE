module aceProtonNuclide_class

  use numPrecision
  use endfConstants
  use universalVariables
  use genericProcedures, only : fatalError, numToChar, charToInt, binarySearch
  ...
  use ceProtonNuclide_inter,        only : ceProtonNuclide, kill_super => kill
  ...
  !! Public Pointer Cast
  !!
  public aceProtonNuclide_CptrCast
  public aceProtonNuclide_TptrCast
  ...
  !!
  !! CE Proton Nuclide Implementation that follows directly from the specification of ACE data
  !!
  !! Public Members:
  !!   ZAID             -> ZZAAA.TTc ID of the ACE card of the nuclide
  !!   eGrid            -> Energy grid for the XSs
  !!   mainData         -> Array of XSs that are required in ceProtonMicroXSs, that is
  ...
  !!   microXSs         -> return interpolated ceProtonMicroXSs package given index and inter. factor
  !!   init             -> build nuclide from aceCard
  !!   display          -> print information about the nuclide to the console
  !!
  type, public, extends(ceProtonNuclide) :: aceProtonNuclide
...
contains
  ...
  !!
  !! See ceProtonNuclide documentation
  !!
  function invertInelastic(self, E, rand) result(MT)
    ...
    character(100), parameter :: Here = 'invertInelastic (aceProtonNuclide_class.f90)'
  ...
  function xsOf(self, MT, E) result(xs)
    ...
    character(100), parameter :: Here = 'xsOf (aceProtonNuclide_class.f90)'
  ...
  elemental subroutine kill(self)
    class(aceProtonNuclide), intent(inout) :: self
    integer(shortInt)                      :: i
  ...
  subroutine search(self, idx, f, E)
    ...
    real(defReal), intent(in)            :: E
    character(100), parameter :: Here = 'search (aceProtonNuclide_class.f90)'
  ...
  !!
  !! Args:
  !!   sigma1 [out] -> first moment of Bethe Block model [MeV * barn]
  !!   sigma2 [out] -> second moment of Bethe Block model [MeV^2 * barn]
  !!   E [in]       -> particle energy [MeV]
  !!   I [in]       -> mean material excitation energy
  !!
  elemental subroutine betheBloch(self, sigma1, sigma2, E, I)
    class(aceProtonNuclide), intent(in) :: self
    real(defReal), intent(out)          :: sigma1
    real(defReal), intent(out)          :: sigma2
    real(defReal), intent(in)           :: E
  ...
  !!   database [in] -> pointer to ceProtonDatabase to set XSs to cache
  !!
  !! Errors:
  !!   FatalError if ACE card has NU data but no fission MTs
  !!
  subroutine init(self, ACE, nucIdx, Z, database)
    class(aceProtonNuclide), intent(inout)        :: self
    class(aceCard), intent(inout)                 :: ACE
    integer(shortInt), intent(in)                 :: nucIdx
    integer(shortInt), intent(in)                 :: Z
    class(ceProtonDatabase), pointer, intent(in)  :: database
  ...
  !!
  !! Cast nuclideHandle pointer to aceProtonNuclide type pointer
  !!
  !! Args:
  !!   source [in]    -> source pointer of class nuclideHandle
  !!
  !! Result:
  !!   Null is source is not of aceProtonNuclide type
  !!   Pointer to source if source is aceNuclearDatabase type
  !!
  pure function aceProtonNuclide_TptrCast(source) result(ptr)
    class(nuclideHandle), pointer, intent(in) :: source
    type(aceProtonNuclide), pointer           :: ptr

    select type(source)
      type is(aceProtonNuclide)
        ptr => source
  ...
  end function aceProtonNuclide_TptrCast

  !!
  !! Cast nuclideHandle pointer to aceProtonNuclide class pointer
  !!
  !! Args:
  !!   source [in]    -> source pointer of class nuclideHandle
  !!
  !! Result:
  !!   Null is source is not of aceProtonNuclide class
  !!   Pointer to source if source is aceNuclearDatabase class
  !!
  pure function aceProtonNuclide_CptrCast(source) result(ptr)
    class(nuclideHandle), pointer, intent(in) :: source
    class(aceProtonNuclide), pointer          :: ptr

    select type(source)
      class is(aceProtonNuclide)
        ptr => source
  ...
  end function aceProtonNuclide_CptrCast


end module aceProtonNuclide_class
