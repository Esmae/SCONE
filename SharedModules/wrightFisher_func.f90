...
contains
  ...
  function func1(nm, numBlocks, thetaSBM) result(s)
      ...
      s = log(thetaSBM+2*nm-1)+log_gamma(thetaSBM+numBlocks+nm-1)-log_gamma(thetaSBM+numBlocks)-log_gamma(numBlocks+1)-log_gamma(nm-numBlocks+1)
  end function func1
  ...
  function func3(numBlocks, sigEsq, thetaSBM) result(n)
      ...
      real(refReal), intent(in)       :: sigEsq, thetaSBM
      integer(shortInt)               :: n
   ...
   !!
   function numberOfBlocks(sigEsq, rand) result(numBlocks) !m
    real(refReal), intent(in)         :: sigEsq ! t
    class(RNG), intent(inout)         :: rand
    integer                           :: i
    logical                           :: proceed
    real(refReal)                     :: thetaSBM, sigma, r1, r2
    real(refReal)                     :: smin, smax, increment
    ...
    if (sigEsq < 0.07) then
        ...
        do while (proceed)
            ...
            k(numBlocks+1) = ceiling(func3(numBlocks,sigEsq,thetaSBM)/2)
            ...
            do while (smin < r1 .and. r1 < smax)
                do i = 0, numBlocks
                    ...
                    smax = smin + increment
                    smin = smin + increment - b(i+2*k(i+1) + 1,i,sigEsq,thetaSBM)
                end do
            end do
  ...
  subroutine getSBMDirection(dirIn, dirOut, wF)
      ...
      if (denom > 1E-10) then
          ...
          if dirIn[1] .ne. 0 then
              ...
              if rxy2 < 1E-10 then
                  ! x << y
                  arr1 = [SIGN(1,-1*dirIn[1]),0,0]
              else if rxy > 1E+10 then
                  ! y << x
                  arr1 = [0,SIGN(1,-1*dirIn[2]),0]
              else
                  arr1 = [SIGN(sqrt(1/(1+rxy2)), -1*dirIn[1]),SIGN(sqrt(rxy2/(rxy2+1)), -1*dirIn[2]),0]
              end if
          else 
              if dirIn[2] .ne. 0 then
                  ...
                  arr1 = [SIGN(1/sqrt(2), -1*dirIn[1]),SIGN(1/sqrt(2), -1*dirIn[2]),0]
          end if
      end if
