...
module transportOperatorSBM_class
  ...
  use wrightFisher_func           only : wright_fisher, getSBMDirection
...
contains
  ...
  !!
  subroutine sphericalBrownianMotion(self, p, dx, tally, thisCycle, nextCycle)
    class(transportOperatorST), intent(inout) :: self
    class(particle), intent(inout)            :: p
    type(tallyAdmin), intent(inout)           :: tally
    class(particleDungeon),intent(inout)      :: thisCycle
    class(particleDungeon),intent(inout)      :: nextCycle
    type(defReal), intent(in)                 :: dx ! max length of SBM step
    integer(shortInt)                         :: event
    real(defReal)                             :: sigmaT, dist, sigma, rad_wr, eLoss
    ...
    STLoop: do
      ...
      sigma  = self % xsData % getMoliereVolatility(self, p, p % prePath % matIdx, dist) 
      rad_wr = wright_fisher(sigma**2,p % pRNG) 
      !Get new direction
      call getSBMDirection(p % dir, dirOut, rad_wr)
      call p % point(dirOut)
      !Do energy update given travel distance dist
      eLoss = getEnergyLoss(p, p % prePath % matIdx, dist)
      if eLoss .gt. p % E then
          p % E = 0
          p % isDead = .true.
          ! What should the fate of p be?
      p % E = p % E - eLoss
      ! If energy < cutoff -> can you just assign a fate as absorption, is this used?
