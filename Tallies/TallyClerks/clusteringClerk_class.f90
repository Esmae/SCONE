module clusteringClerk_class
  ...
  !! ejw89 TO CHANGE, and other comments throughout
  !!  clerkName {
  ...
  !!  }
  !!
  type, public, extends(tallyClerk) :: clusteringClerk
    private
...
contains
  ...
  subroutine init(self, dict, name)
    class(clusteringClerk), intent(inout) :: self
    ...
    ! Allocate space for storing number of times get X particles in bin 1
    ! ejw89 TOCHANGE, for now allocate 2000
    allocate(self % value(2000))
    self % value = ZERO
    ...
    ! ejw89 is this needed? As value is probability in this case?
  ...
  function validReports(self) result(validCodes)
    class(clusteringClerk),intent(in)      :: self
  ...
  elemental function getSize(self) result(S)
    ...
    ! ejw89 to change when fixed how big
    ...
    if (self % currentCycle < self % maxCycles) then
      ...
      do i = 1,end % popSize()
        associate( state => end % get(i) )
          ...
          if (state % wgt .NE. 1) print *, 'ejw89 weight not equal to 1'
        end associate
      end do
      ...
      self & value(cc) = self & value(cc) + self & prob(j)
      !
      !do j = 1,self % N
  ...
  subroutine display(self, mem)
    ...
    print *, 'clusteringClerk does not support display yet'
...
end module clusteringClerk_class
